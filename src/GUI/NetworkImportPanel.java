...
public class NetworkImportPanel extends JPanel{
	...
	{
		Button multipleBtn = new Button("Import multiple files");
		Button okBtn = new Button("Ok");
		Button cancelBtn = new Button("Cancel");
		
		multipleBtn.setBounds(190, 180, 160, 30);
		this.add(multipleBtn);
		...
		multipleBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				NetworkImportPanel.this.frame.dispose();
				NetworkImportPanel.this.openFileChooser1();
			}
		});
	...
	private void openFileChooser()
	{
		...
		if (fd.getDirectory() != null && fd.getFile() != null) {
			try {
				...
				parentPanel.parentFrame.enableConstructionMenus();
				parentPanel.parentFrame.enableNetworkMenus();
	...
	private void openFileChooser1()
	{
		...
		JPanel[] jp = new JPanel[100];
		parentPanel.parentFrame.enableConstructionMenus();
		parentPanel.parentFrame.enableNetworkMenus();
		  for(int j = 0; j<nof; j++){
			path = files[j].getAbsolutePath();
			if(chooser.getCurrentDirectory() != null && file!= null){
				try {
					System.out.println(path);
					Network net = new Network(this.mode);
					int x = net.loadNetwork(isDirected, path, headers); 
					...
					if(firstLoad)
						parentPanel.parentFrame.toggleNetworkMenus();
					parentPanel.netgraphlist.add(new NetworkGraph(net));
				}
				catch (MyOwnException e) {
					parentPanel.parentFrame.netPanel = null;
					parentPanel.parentFrame.reset();
		  ...
		  parentPanel.netGraph = parentPanel.netgraphlist.get(0);
		  parentPanel.loadGraph();
