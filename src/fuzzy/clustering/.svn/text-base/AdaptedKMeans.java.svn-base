 ...
 */
public class AdaptedKMeans implements KMeans {
    ...
    private KMeansCluster[] mClusters;

    // An array of Cluster objects: the output of k-means.
    private Cluster[] mInitialClusters;
     ...
     */
    public AdaptedKMeans(double[][] coordinates, int k, int maxIterations, 
            long randomSeed) {
    ...
    public AdaptedKMeans(double[][] coordinates, Cluster[] initialClusters, int k, int maxIterations, 
            long randomSeed) {
        mCoordinates = coordinates;
        // Can't have more clusters than coordinates.
        mK = Math.min(k, mCoordinates.length);
        mMaxIterations = maxIterations;
        mRandomSeed = randomSeed;
        mInitialClusters = initialClusters;
    }
     ...
     */
    private void postKMeansComplete(KMeansCluster[] clusters, long executionTime) {
        if (mListeners.size() > 0) {
            synchronized (mListeners) {
                int sz = mListeners.size();
                for (int i=0; i<sz; i++) {
     ...
     */
    public void runWithInitialClustersAssigned() {
        try {
            ...
            makeAssignments(mInitialClusters);
            
            System.out.println("... centers and members initialized");
...
//              postKMeansMessage("... iteration " + it + " moves = " + moves);
    ...
    private int makeAssignments(Cluster[] initClusters)
    {
//        Random random = new Random(mRandomSeed);
        ...
        if (mClusterAssignments == null) {
        	...
        	double[] myCenter = new double[1];
            for (int i=0; i<initClusters.length; i++)
            {
            	myCenter[0] = initClusters[i].centroid();
            	mProtoClusters[i] = new ProtoCluster(myCenter, initClusters[i].getM_ElementIdx()[0]);
        		mClusterAssignments[initClusters[i].getM_ElementIdx()[0]] = initClusters[i].getLabel();
            	for (int j=1; j<initClusters[i].getNumElements(); j++)
            	{
            		mProtoClusters[i].add(initClusters[i].getM_ElementIdx()[j]);
            		mClusterAssignments[initClusters[i].getM_ElementIdx()[j]] = initClusters[i].getLabel();
            	}
            }
        }
        try {
    ...
    /**
     *  the nearest cluster to the coordinate identified by
     * the specified index.
     */
     ...
     */
    private KMeansCluster[] generateFinalClusters() {
        ...
        List<KMeansCluster> clusterList = new ArrayList<KMeansCluster>(numClusters);
        for (int c = 0; c < numClusters; c++) {
            ProtoCluster pcluster = mProtoClusters[c];
            if (!pcluster.isEmpty()) {
            	KMeansCluster cluster = new KMeansCluster(pcluster.getMembership(), pcluster.getCenter());
                clusterList.add(cluster);
            }
        }
    
        // - convert list to an array.
        KMeansCluster[] clusters = new KMeansCluster[clusterList.size()];
