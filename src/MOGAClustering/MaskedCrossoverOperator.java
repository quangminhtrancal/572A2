...
public class MaskedCrossoverOperator 
extends BaseGeneticOperator implements Comparable{
	 ...
	 * @since 2.0
	 */
	public MaskedCrossoverOperator()
	throws InvalidConfigurationException {
	...
	public MaskedCrossoverOperator(Configuration a_configuration)
	throws InvalidConfigurationException {
	...
	protected void doCrossover(IChromosome firstMate, IChromosome secondMate,
		...
		for (int j = 0; j < effectiveLength; j++) {
			...
			try {
				if ((Double) gene1.getAllele() - (Double) gene2.getAllele() == 0) {
					newGenes1[j] = new DoubleGene(gene1.getConfiguration());
					newGenes1[j].setAllele(gene1.getAllele());
					newGenes2[j] = new DoubleGene(gene2.getConfiguration());
					newGenes2[j].setAllele(gene2.getAllele());
				}
				else
				{
					...
					{
						newGenes1[j] = new DoubleGene(gene1.getConfiguration());
						newGenes1[j].setAllele(gene2.getAllele());
						newGenes2[j] = new DoubleGene(gene2.getConfiguration());
						newGenes2[j].setAllele(gene1.getAllele());
					}
					else
					{
						newGenes1[j] = new DoubleGene(gene1.getConfiguration());
